...
int main( void ) {
    ...
    // intialize the constant data using a triangles limits
    float *temp = (float*)malloc( imageSize );
    for (int i=0; i<DIM*DIM; i++) {
        temp[i] = 0;
        int x = i % DIM;
        int y = i / DIM;
        if (y>=x/2 && y>=-x/2+DIM/2){ //Set to a triangle bound
            temp[i] = MAX_TEMP;
        }
    }
    ...
    // initialize the input data using a triangle's limits
    for (int i=0; i<DIM*DIM; i++) {
        int x = i % DIM;
        int y = i / DIM;
        if (y>=x/2 && y>=-x/2+DIM/2){
            temp[i] = MAX_TEMP;
        } else{
            temp[i] = MIN_TEMP;
        }
